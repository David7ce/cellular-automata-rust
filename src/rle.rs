...
mod tests {
    ...
    fn writes_the_documented_golly_form() {
        let r_pentomino = cells("b2o$2o$bo!")
            .iter()
            .map(|&(x, y)| (x as i64, y as i64))
            .collect::<Vec<_>>();
        assert_eq!(
            to_rle(r_pentomino, life()),
            "x = 3, y = 3, rule = B3/S23\nb2o$2o$bo!\n"
        );
    ...
    fn export_wraps_at_70_columns_and_round_trips() {
        ...
        assert_eq!(
            normalized(back.cells.iter().map(|&(x, y)| (x as i64, y as i64))),
            normalized(cells)
        );
    }
}
